...
public class InputStreamAssert<SELF, ACTUAL, ACTUAL_VALUE> extends AbstractAssert<InputStreamAssert<SELF, ACTUAL, ACTUAL_VALUE>, ACTUAL, byte[]> {
  ...
  public InputStreamAssert isSameAs(byte[] expected) throws AssertionError {
    try {
      if (!IoUtil.areInputStreamsEqual(new ByteArrayInputStream(actualValue.get()),
  ...
  public InputStreamAssert isNotSameAs(byte[] expected) throws AssertionError {
    try {
      isSameAs(expected);
