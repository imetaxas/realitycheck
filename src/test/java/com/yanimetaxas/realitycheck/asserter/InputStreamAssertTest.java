...
public class InputStreamAssertTest {
  ...
  public void isSameAs_True() throws Exception {
    ...
    assertNotNull(Reality.checkThat(new ByteArrayInputStream(bytes)).hasSameContentAs(new ByteArrayInputStream(bytes)));
  }

  @Test(expected = AssertionError.class)
  public void isSameAs_False() throws Exception {
    byte[] bytes = "RandomString".getBytes();
    Reality.checkThat(new ByteArrayInputStream(bytes)).hasNotSameContentAs(new ByteArrayInputStream(bytes));
  }
  ...
  public void isNotSameAs_True() throws Exception {
    ...
    assertNotNull(Reality.checkThat(new ByteArrayInputStream(bytes1)).hasNotSameContentAs(new ByteArrayInputStream(bytes2)));
  }
